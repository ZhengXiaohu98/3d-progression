...
export const Book: React.FC<BookProps> = ({ pageColor = "#ededed", className, children }) => {
  ...
  return (
    <div
      className={cn("transition-all flex relative group/book rounded !overflow-visible", className)}
      style={{
        perspective: "1000px",
      }}
    >
      ...
      >
        ...
        <button className={cn("absolute bottom-4 right-4 z-10 opacity-100 lg:group-hover/book:opacity-100 lg:opacity-0 transition-all ease-in-out !pointer-events-auto")} onClick={() => { setPageState(pageState ^ 1) }}>
