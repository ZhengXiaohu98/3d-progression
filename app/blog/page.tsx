        ...
        <div className={cn("transition-all md:w-full lg:w-[816px] shrink-0 flex flex-col gap-x-4 gap-y-8")}>
          {blogData.map(blog => (
            <Link key={blog.title} href={blog.url}>
              <div className={cn("blog-div group flex shrink-0 relative sm:flex-col md:flex-row gap-1 items-center w-full border-b dark:border-accent-300 border-accent-ccc")}>
                <div className="lg:w-[400px] lg:h-[200px] md:w-[320px] md:h-[160px] sm:w-[340px] sm:h-[170px] overflow-hidden shrink-0 rounded-sm">
                ...
                <div className="px-2 py-4">
                  <div className="flex items-center gap-1 mb-2">
                    {blog.categories.map((category) => (
                      <div key={category} className="px-1.5 sm:py-0.5 md:py-1 dark:bg-gray-dark bg-gray-light text-white dark:text-accent-white rounded-sm text-xs tracking-wide italic">
                        {category}
                      </div>
                    ))}
                  </div>
                  <h2 className="md:text-base lg:text-xl leading-none sm:text-sm italic font-bold text-accent-dark dark:text-accent-white">
                    <span className="duration-500 bg-gradient-to-r from-orange-base to-pink-base bg-no-repeat bg-[length:0px_1px] group-hover:bg-[length:100%_1px] bg-right-bottom group-hover:bg-left-bottom" style={{ transitionProperty: "background-size" }}>
                      {blog.title}
                    </span>
                  </h2>
