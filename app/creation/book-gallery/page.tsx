...
const BookGallery: React.FC = () => {
  ...
  useGSAP(() => {
    ...
    const distance = (elements[0] as HTMLElement).offsetWidth;
  ...
  return (
    <div className="overflow-x-hidden">
      <div
        className="fixed top-0 left-0 h-screen w-screen select-none transition-all"
        style={{
          background: theme === "dark"
            ? "radial-gradient(#fdfdfd, #232323 80%)"
            : "radial-gradient(#ffffff, #fdfdfd 80%)",
        }}
      >
        <Suspense fallback={<ProgressLoader />}>
          <Canvas shadows camera={{ position: [-0.5, 1, 3], fov: 45 }}>
            <Float rotation-x={Math.PI / 30} floatIntensity={1} speed={2} rotationIntensity={1}>
              <Book rotation={[-Math.PI / 4, -Math.PI / 2, 0]} curPage={curPage} setCurPage={setCurPage} />
            </Float>
            <OrbitControls enableZoom={false} minAzimuthAngle={-Math.PI / 4} maxAzimuthAngle={Math.PI / 4} />
